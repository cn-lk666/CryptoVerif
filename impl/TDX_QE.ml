   ...
   begin
     ...
     fun (input_15) ->
       ...
       begin
         ...
         if exists_in_table "mes_TDXM_TD_8"
           (function
               | [tvar_18] -> begin
         
                 let var_tdr_5fTDXM_2 = (exc_bad_file "mes_TDXM_TD_8" id tvar_18) in 
                 if (((=) var_tdr_5fQE_0 var_tdr_5fTDXM_2)) then () else raise Match_fail
                 end
               | _ -> raise (Bad_file "mes_TDXM_TD_8")) then begin 
           try
             let bvar_19=var_tdr_5fQE_0 in
             let (var_smr_5fQE_0,var_res4_5fQE_0,var_tdi_5fQE_0)=inv_tdr_f bvar_19 in
             if true then begin 
               try
                 let bvar_20=var_smr_5fQE_0 in
                 let (var_rms_5fQE_0,var_tcbi_5fQE_0)=inv_smr_f bvar_20 in
                 if true then begin 
                   try
                     let bvar_21=var_rms_5fQE_0 in
                     let (var_rms_5fwithout_5fmac_5fQE_0,var_macres_5fQE_0)=inv_rms_f bvar_21 in
                     if true then begin 
                       try
                         let bvar_22=var_rms_5fwithout_5fmac_5fQE_0 in
                         let (var_rtyp_5fQE_0,var_res1_5fQE_0,var_csvn_5fQE_0,var_tdih_5fQE_0,var_tcbh_5fQE_0,var_rdata_5fQE_0,var_res2_5fQE_0)=inv_rms_without_mac_f bvar_22 in
                         if true then begin 
                           insert_in_table "mes_QE_CPU_12" [(id (var_rms_5fQE_0))];

                           if ((&&) ((=) var_tdih_5fQE_0 (Ssh_crypto.hash var_hk1_0 (tdi_t2bitstring var_tdi_5fQE_0))) ((=) var_tcbh_5fQE_0 (Ssh_crypto.hash var_hk2_0 (tcbi_t2bitstring var_tcbi_5fQE_0)))) then
                           begin
                             (
                              begin
                                let token_23 = ref true in
                                fun () ->
                                  if (!token_23) then
                                  begin
                                    token_23 := false;
                                    if exists_in_table "mes_CPU_QE_14"
                                      (function
                                          | [tvar_25] -> begin
                                            ...
                                            let var_rms_5fwithout_5fmac_5ffrom_5fCPU_2 = (exc_bad_file "mes_CPU_QE_14" id tvar_25) in 
